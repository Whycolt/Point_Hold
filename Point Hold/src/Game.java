...
public class Game extends JPanel implements Runnable, KeyListener, ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7951651510338024933L;
	/****************************************************
	 * Variable Dictionary isOpen - Boolean used to keep loop open line - Used
	 * to store line read by reader a - Used to store random enemy menu num -
	 * Used to find available enemy lot in varList varList - List used to store
	 * enemy information (x,y,health,direction) bulList - List used to store
	 * friendly bullet information(x,y,direction) scores - List used to store
	 * scores avaliUnits - List used to find available enemies slots playerHP -
	 * Used to store player health playerMove - Used to store player direction
	 * playerx - Used to store players x coordinate playery - Used to store
	 * players y coordinate shootDir - Used to store shooting direction
	 * colorTime - Used to store color of the time bar menu - Used to store menu
	 * menu TxtActive - Used to check if JTextfield is open gameTimer - Used to
	 * store game start time spawnTimer - Used to store the time of last spawn
	 * spawnDelay - Used to store delay between enemy spawns score - Used to
	 * store score of current game shooting - Boolean to determine if player is
	 * shooting btnGame - Play button btnStand - Standard mode button btnCheat -
	 * Cheat mode button btnInst - Instruction menu button btnScore - Score menu
	 * button btnBack - Back button btnEnter - Enter name button lblScore1 -
	 * Used to write first place score lblScore2 - Used to write second place
	 * score lblScore3 - Used to write third place score lblScore4 - Used to
	 * write fourth place score lblScore5 - Used to write fifth place score
	 * lblScore6 - Used to write sixth place score lblScore7 - Used to write
	 * seventh place score lblScore8 - Used to write eighth place score
	 * lblScore9 - Used to write ninth place score lblScore10 - Used to write
	 * tenth place score lblScoreCur - Used to write current game score
	 * lblHealth - Used to write player health lblTime - Used to write time
	 * remaining txtPlayer - Used to enter player name reader - Buffered reader
	 * writer - Buffered writer s -Used to store strings when reading from txt
	 * file playername - Used to store name entered by player filePath - Used to
	 * store src file path t - game thread
	 ****************************************************/
	...
	public void hitdetect() {
		...
		for (int i = 1; i < 96; i++) {
			if (varList[i][2] > 0) {
				if (((varList[i][0] <= playerx && varList[i][0] + 50 >= playerx)
						|| (varList[i][0] <= playerx + 50 && varList[i][0] + 50 >= playerx + 50))
						&& ((varList[i][1] <= playery && varList[i][1] + 50 >= playery + 50)
								|| (varList[i][1] <= playery + 50 && varList[i][1] + 50 >= playery + 50))) {
			...
			for (int j = 0; j < 9; j++) {
				if (varList[i][2] > 0) {
					if (((varList[i][0] <= bulList[j][0] && varList[i][0] + 50 >= bulList[j][0])
							|| (varList[i][0] <= bulList[j][0] + 10 && varList[i][0] + 50 >= bulList[j][0] + 10))
							&& ((varList[i][1] <= bulList[j][1] && varList[i][1] + 50 >= bulList[j][1] + 50)
									|| (varList[i][1] <= bulList[j][1] + 10
											&& varList[i][1] + 50 >= bulList[j][1] + 10))
							&& i < 16) {
